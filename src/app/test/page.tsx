...
interface TransformMessageInfo<ChatMessage, Output> {
...
const DEFAULT_CONVERSATIONS_ITEMS = [{ key: 'default-0', label: '默认会话' }]
...
> extends AbstractChatProvider<ChatMessage, Input, Output> {
  ...
  transformMessage(info: TransformMessageInfo<ChatMessage, Output>): ChatMessage {
    ...
    // chunk 是 Output 类型，需要安全地访问 chunk.data
    const chunkData =
      chunk && typeof chunk === 'object' && 'data' in chunk
        ? ((chunk as Record<string, unknown>).data as string | undefined)
        : undefined

    if (!chunkData || typeof chunkData !== 'string' || chunkData.includes('[DONE]')) {
    ...
    try {
      ...
      return {
        content: `${originMessage?.content || ''}${chunkData}`,
        role: 'assistant'
      } as ChatMessage
    }
  }
...
// 从后端获取会话历史消息
const getApiHistoryMessages = async (
...
const Independent: React.FC = () => {
  ...
  // 缓存Provider
  const providerCaches = useRef<
    Map<string, CustomProvider<CustomMessage, CustomInput, CustomOutput>>
  >(new Map())
  ...
  const getApiDefaultConversations = async () => {
    try {
      ...
      if (
        ...
        const conversations = response.data.sessions.map((item) => ({
          key: item.session_id,
        ...
        if (
          !activeConversationKey ||
          !conversations.find((c: { key: string }) => c.key === activeConversationKey)
        ) {
    ...
    try {
      ...
      if (response.code === 200 && response.data?.session_id) {
        ...
        addConversation({
          key: newSessionId,
          label:
            response.data.session_name || `${locale.newConversation} ${conversations.length + 1}`
        })
  ...
  // 加载会话列表
  useEffect(() => {
    if (isClient) getApiDefaultConversations()
  }, [isClient])
  ...
  const getDefaultMessages: (info: {
    ...
    try {
      return await getApiHistoryMessages(conversationKey)
    } catch (error) {
  ...
  const currentProvider =
    activeConversationKey && isClient ? getProvider(activeConversationKey) : undefined

  const { onRequest, messages, isRequesting, abort, onReload, setMessage } = useXChat({
    provider: currentProvider as
      | AbstractChatProvider<CustomMessage, CustomInput, CustomOutput | undefined>
      | undefined,
  ...
  // 切换会话清空输入框
  useEffect(() => {
    if (isClient) senderRef.current?.clear()
  }, [activeConversationKey, isClient])
  ...
  const Footer: React.FC<{
    ...
    const isLastMessage =
      ...
      String(lastAssistant.id) === String(id) &&
              ...
              {content.map((item, index) => {
                if (item.type === 'image' && item.image_url) {
                  ...
                  return (
                    <div
                      key={index}
                      style={{ position: 'relative', display: 'inline-block', margin: '8px 0' }}
                    >
                      <img
                        src={item.image_url}
                        alt="uploaded"
